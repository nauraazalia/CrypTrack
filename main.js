...
searchBar.addEventListener('input', function () {
    const query = searchBar.value.toLowerCase(); // Ambil input pengguna dan ubah ke huruf kecil
    const allCoins = document.querySelectorAll('.coin-detail'); // Pilih semua elemen koin

    allCoins.forEach(coin => {
        const coinName = coin.querySelector('.coin-title').textContent.toLowerCase();
        const coinSymbol = coin.querySelector('.symbol-name').textContent.toLowerCase();
        
        // Tampilkan atau sembunyikan koin berdasarkan input
        if (coinName.includes(query) || coinSymbol.includes(query)) {
            coin.style.display = 'flex'; // Tampilkan koin
        } else {
            coin.style.display = 'none'; // Sembunyikan koin
        }
    });
});
